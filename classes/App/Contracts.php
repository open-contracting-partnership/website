...
class Contracts {
	...
	public function fetch_contracts() {
		...
		foreach ( $contracts->releases as $contract ) {
			...
			if ( isset($contract->contracts[0]->implementation->milestones) ) {
	...
	private function save_to_json($data) {
		...
		file_put_contents(wp_upload_dir()['basedir'] . '/contracts/contracts.json', $content);
	}
	...
	private function save_to_csv($data) {
		...
		$fp = fopen(wp_upload_dir()['basedir'] . '/contracts/contracts.csv', 'w');
		$first = true;
